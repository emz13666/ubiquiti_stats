...
begin
  ...
  begin
      ...
      query.SQL.Clear;
      query.SQL.Add('select count(s.id) as num from statss s, equipment e, modems m where (e.equipment_type in (1,2))  and (e.id=m.id_equipment) and (s.id_modem=m.id_modem) and (s.status=0)');
      try
         query.Active:=true;
      except
      end;
      ...
      query.Active:=false;
      query.SQL.Clear;
      query.SQL.Add('select s.id, s.signal_level, s.date, s.time, s.status, m.name from statss s, modems m, equipment e ');
      query.SQL.Add('where (e.equipment_type in (1,2)) and (e.id=m.id_equipment) and (s.id_modem=m.id_modem) and (s.status=0) order by s.date');
      try
         query.Active:=true;
      except
      end;
